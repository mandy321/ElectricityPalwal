...
while ((match = polyRegex.exec(content)) !== null) {
  ...
  if (body.includes('class="st30"')) {
    ...
    if (idMatch && pointsMatch) {
      ...
      if (centroid) shapes.push({ type: 'polygon', id: idMatch[1], centroid });
    }
  }
}
...
while ((match = pathRegex.exec(content)) !== null) {
  ...
  if (body.includes('class="st30"')) {
    ...
    if (idMatch && dMatch) {
      ...
      if (centroid) shapes.push({ type: 'path', id: idMatch[1], centroid });
    }
  }
}
...
labels.forEach(label => {
  const sorted = shapes.map(shape => {
    ...
    return { id: shape.id, type: shape.type, dist, centroid: shape.centroid };
  }).sort((a, b) => a.dist - b.dist);
  
  console.log(`District Label: ${label.name} at [${label.x.toFixed(1)}, ${label.y.toFixed(1)}]`);
  console.log(`  1. Shape ID: ${sorted[0].id} (dist: ${sorted[0].dist.toFixed(1)})`);
  console.log(`  2. Shape ID: ${sorted[1].id} (dist: ${sorted[1].dist.toFixed(1)})`);
  console.log(`  3. Shape ID: ${sorted[2].id} (dist: ${sorted[2].dist.toFixed(1)})`);
});
